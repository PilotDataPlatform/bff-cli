...
async def test_hpc_auth_should_return_200(test_async_client, httpx_mock):
    ...
    res = await test_async_client.post(
        '/v1/hpc/auth',
        headers=header,
        json=payload)
    response = res.json()
...
async def test_hpc_auth_with_error_token_should_return_200(
    test_async_client,
    httpx_mock
):
    payload = {
...
async def test_submit_hpc_job_without_script_should_return_400(
    test_async_client
):
    payload = {
        'host': 'http://host',
    ...
    header = {'Authorization': 'fake token'}
    res = await test_async_client.post(
        test_api, headers=header, json=payload)
    response = res.json()
...
async def test_hpc_get_job_success_should_return_200(
    test_async_client,
    httpx_mock
):
    params = {
    ...
    httpx_mock.add_response(
        method='GET',
        url=(
            'http://service_hpc/v1/hpc/job/12345?'
            'slurm_host=host&username=username&protocol=http'),
        json={'result': {
            'job_id': '12345',
            'job_state': 'COMPLETED',
            'standard_error': '',
    ...
    res = await test_async_client.get(
        test_api, headers=header, query_string=params)
    response = res.json()
    assert response.get('code') == 200
    assert response.get('error_msg') == ''
...
async def test_hpc_get_job_wrong_id_should_return_404(
    test_async_client,
    httpx_mock
):
    params = {
    ...
    httpx_mock.add_response(
        method='GET',
        url=(
            'http://service_hpc/v1/hpc/job/123?'
            'slurm_host=host&username=username&protocol=http'),
...
async def test_hpc_list_nodes_without_protocal_should_return_404(
    test_async_client,
    httpx_mock
):
    params = {
...
async def test_hpc_get_node_with_node_name_should_return_200(
    test_async_client,
    httpx_mock
):
    params = {
    ...
    httpx_mock.add_response(
        method='GET',
        url=(
            'http://service_hpc/v1/hpc/nodes/fake_name?'
            'slurm_host=host&username=username&protocol=http'),
...
async def test_hpc_get_node_without_node_name_should_return_404(
    test_async_client,
    httpx_mock
):
    params = {
...
async def test_hpc_list_partitions_should_return_200(
    test_async_client,
    httpx_mock
):
    params = {
    ...
    httpx_mock.add_response(
        method='GET',
        url=(
            'http://service_hpc/v1/hpc/partitions?'
            'slurm_host=host&username=username&protocol=http'),
        json={
            'result': [
                {'partition_name1': {'nodes': ['fake_node']}},
                {'partition_name2': {'nodes': ['fake_node2']}}],
            'code': 200},
        status_code=200,
    )
...
async def test_hpc_list_partitions_without_protocal_should_return_400(
    test_async_client,
    mocker
):
    params = {
...
async def test_hpc_get_partition_by_name_should_return_200(
    test_async_client,
    httpx_mock
):
    params = {
    ...
    httpx_mock.add_response(
        method='GET',
        url=(
            'http://service_hpc/v1/hpc/partitions/fake_name?'
            'slurm_host=host&username=username&protocol=http'),
...
async def test_hpc_get_partition_by_name_without_protocal_should_return_400(
    test_async_client,
    mocker
):
    params = {
