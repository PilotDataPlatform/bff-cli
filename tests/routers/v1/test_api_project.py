...
test_get_project_item_api = '/v1/project/test_project/search'
...
async def test_upload_files_into_project_should_return_200(
    test_async_client_auth, mocker, httpx_mock
):
    payload = {
        'operator': 'test_user',
        'upload_message': 'test',
        ...
        'data': [
            {'resumable_filename': 'fake.png', 'resumable_relative_path': 'folder1'}
        ],
    }
    mocker.patch(
            ...
            "result": [
                {
                    ...
                    "container_code": "test_project",
                    "container_type": "project",
                    "created_time": "2022-06-06 15:21:55.723898",
                    "last_updated_time": "2022-06-06 15:21:55.723911",
                    "storage": {
                        "id": "storage-id",
                        "location_uri": "fake-minio-path",
                        "version": "version-id"
                    },
                    "extended": {
                        "id": "test-extended-id",
...
async def test_get_folder_in_project_should_return_200(
    ...
    res = await test_async_client_auth.get(
        test_get_project_item_api, headers=header, query_string=param
    )
    res_json = res.json()
...
async def test_get_file_in_project_should_return_200(
    test_async_client_auth, mocker, httpx_mock: HTTPXMock
):
    param = {
        'zone': 'zone',
...
async def test_get_folder_in_project_without_token_should_return_401(
    ...
    res = await test_async_client.get(
        test_get_project_item_api, query_string=param
    )
    res_json = res.json()
    assert res_json.get('code') == 401
    assert res_json.get('error_msg') == 'Token required'
